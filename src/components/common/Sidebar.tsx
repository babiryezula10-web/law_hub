// ─────────────────────────────────────────────────────────────
// LawHub Uganda — Common UI: Sidebar
// Left navigation drawer with role-filtered links & search targets.
// ─────────────────────────────────────────────────────────────
    ...
    <div className="flex flex-col h-full justify-between py-4 px-3">
      ...
      <div>
        <div className="flex items-center justify-between px-2 mb-4">
          ...
          <button
            onClick={onClose}
            className="md:hidden p-1.5 rounded-lg text-slate-400 hover:text-slate-100 hover:bg-[#0e172a] transition cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                      <Icon
                        className={`w-4 h-4 shrink-0 ${
                          isActive ? 'text-[#050811]' : 'text-slate-400'
                        }`}
                      />
      ...
      <div className="pt-3 mt-auto border-t border-white/10 px-2 space-y-2">
        <div className="p-3 bg-white/[0.04] border border-white/10 rounded-2xl text-center space-y-1.5 backdrop-blur-md">
          ...
          <button
            ...
            className="w-full py-1.5 rounded-xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-heading font-extrabold text-[11px] transition shadow-sm cursor-pointer active:scale-[0.98]"
          >
            Launch Tutor
          </button>
        </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex">
          <div
            ...
            onClick={onClose}
          />
          <aside className="relative w-72 max-w-[85vw] bg-[#070b16]/95 backdrop-blur-2xl border-r border-white/10 text-slate-100 flex flex-col h-full z-10 shadow-2xl animate-in slide-in-from-left duration-300">
            {renderContent()}
          </aside>
        </div>
      )}
