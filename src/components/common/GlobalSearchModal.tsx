// ─────────────────────────────────────────────────────────────
// LawHub Uganda — Common UI: GlobalSearchModal
// Omnipresent search overlay indexing Constitution, statutes,
// cases, academic courses, past examination papers, and notes.
// ─────────────────────────────────────────────────────────────
...
export function GlobalSearchModal({
  ...
  const allResults = useMemo(() => {
    ...
    statutesList.forEach((st: Statute) => {
      if (
        ...
        st.sections.some(
          (sec) =>
            sec.title.toLowerCase().includes(q) || sec.text.toLowerCase().includes(q),
        )
      ) {
        results.push({
          id: st.id,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-12 sm:pt-16 px-3 sm:px-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200 text-slate-100">
      ...
      <div className="relative w-full max-w-3xl bg-[#09090b]/95 border border-white/10 rounded-3xl shadow-2xl overflow-hidden z-10 flex flex-col max-h-[85vh] backdrop-blur-2xl">
        ...
        <div className="p-4 border-b border-white/10 flex items-center gap-3 bg-black/40 backdrop-blur-md">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-slate-100 placeholder-slate-400 text-xs sm:text-sm focus:outline-none font-medium"
            autoFocus
          />
          {query && (
            <button
          ...
          {!query.trim() ? (
            ...
            <div className="text-center py-12 space-y-4">
              ...
              <button
                onClick={() => {
                  onOpenTutorWithPrompt(
                    `Explain legal provisions and cases regarding: "${query}" in Ugandan law.`,
                  );
                  onClose();
                }}
