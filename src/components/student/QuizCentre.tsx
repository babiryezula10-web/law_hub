// ─────────────────────────────────────────────────────────────
// LawHub Uganda — Student Component: QuizCentre
// Practice exam centre with presets, dynamic AI generator, and history.
// ─────────────────────────────────────────────────────────────
    ...
    try {
      ...
      if (res.data?.questions && res.data.questions.length > 0) {
        const newQuiz: Quiz = {
          ...
          questions: res.data.questions as any,
        };
    ...
    <div className="relative space-y-6 sm:space-y-8 pb-12 text-slate-100">
      ...
      <div className="relative z-10 bg-slate-950/40 border border-white/10 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl backdrop-blur-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 bg-black/30 p-2.5 rounded-2xl border border-white/10 text-xs backdrop-blur-sm">
            <Trophy className="w-4 h-4 text-[#c89d42]" />
            <span className="text-slate-200 font-semibold">
              Rank: <strong className="text-[#dfb858]">Top 5% Student</strong>
            </span>
          </div>
        </div>
            ...
            {quizPresets.map((qp) => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-bold transition flex items-center gap-1.5 cursor-pointer backdrop-blur-sm active:scale-[0.98] ${
                    isSelected
        ...
        <form
          onSubmit={handleGenerateAiQuiz}
          className="bg-black/30 p-4 sm:p-5 rounded-2xl border border-[#c89d42]/30 space-y-3 backdrop-blur-sm"
        >
            ...
            <div className="md:col-span-2 flex gap-2 items-end">
              <div className="flex-1">
                <label className="text-[10px] text-slate-400 block mb-1">
                  Specific Topic or Statute Section
                </label>
                <input
                  type="text"
                  value={genTopic}
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-bold text-xs transition flex items-center gap-1.5 whitespace-nowrap shadow-md cursor-pointer active:scale-[0.98]"
              >
                {isGenerating ? (
        ...
        <div className="bg-slate-950/40 border border-white/10 rounded-3xl p-6 sm:p-8 space-y-6 shadow-xl backdrop-blur-md">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
            <div>
              ...
              <h2 className="font-heading font-extrabold text-xl text-slate-100 mt-1">
                {activeQuiz.title}
              </h2>
            </div>
          ...
          {/* Quiz Score Banner */}
          {submitted && (
            <div className="p-6 rounded-2xl bg-black/40 border border-[#c89d42]/30 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left backdrop-blur-sm animate-in fade-in duration-200">
              <div>
                ...
                <h3 className="font-heading font-extrabold text-2xl text-slate-100 mt-1">
                  Score:{' '}
                  <span className="text-[#c89d42]">
                    {score} / {activeQuiz.totalQuestions}
                  </span>{' '}
                  ({Math.round((score / activeQuiz.totalQuestions) * 100)}%)
                </h3>
              ...
              {score >= Math.ceil(activeQuiz.totalQuestions * 0.7) && (
                <a
                  href={`data:text/plain;charset=utf-8,${encodeURIComponent(
                    `LAWHUB UGANDA CERTIFICATE OF COMPLETION\n\nThis certifies that Student has successfully passed the exam "${activeQuiz.title}" with a score of ${score}/${activeQuiz.totalQuestions}.\n\nDate: ${new Date().toLocaleDateString()}\nIssuer: LawHub Examination Board`,
                  )}`}
                  download="LawHub_Quiz_Certificate.txt"
                  className="px-5 py-2.5 rounded-xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-bold text-xs transition flex items-center gap-2 whitespace-nowrap shadow-lg shadow-amber-500/20 cursor-pointer active:scale-[0.98]"
                >
          ...
          <div className="space-y-6">
            {activeQuiz.questions.map((q, idx) => {
              ...
              return (
                <div
                  key={q.id}
                  className="bg-black/30 p-5 rounded-2xl border border-white/10 space-y-4 backdrop-blur-sm"
                >
                  ...
                  {q.options ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {q.options.map((opt, oIdx) => {
                        ...
                        let optionStyle =
                          'bg-white/[0.04] border-white/10 text-slate-300 hover:border-[#c89d42]/40';
                        ...
                        if (submitted) {
                          ...
                          optionStyle =
                            'bg-[#c89d42]/20 border-[#c89d42] text-[#c89d42] font-bold';
                        }
          ...
          {!submitted ? (
            <button
              ...
              className="w-full py-3.5 rounded-2xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-bold text-sm transition shadow-lg cursor-pointer active:scale-[0.98]"
            >
            ...
            <button
              onClick={handleResetQuiz}
              className="w-full py-3.5 rounded-2xl bg-white/[0.05] hover:bg-white/10 border border-white/10 text-slate-100 font-bold text-sm transition cursor-pointer active:scale-[0.98]"
            >
              Retake quiz
            </button>
          )}
      ...
      <div className="bg-slate-950/40 border border-white/10 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl backdrop-blur-md">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-heading font-extrabold text-lg text-slate-100">
              Your Exam Performance History
            </h2>
          </div>
