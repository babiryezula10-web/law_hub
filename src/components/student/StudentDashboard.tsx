// ─────────────────────────────────────────────────────────────
// LawHub Uganda — Student Component: StudentDashboard
// Comprehensive learner workspace with course units, coursework
// submission, grading feedback, and quick study actions.
// ─────────────────────────────────────────────────────────────
...
import { getStudentSubmissions, submitStudentAssignment, deleteStudentSubmission } from '../../services/api';
...
export function StudentDashboard({
  ...
  const [activeDashTab, setActiveDashTab] = useState<
    'overview' | 'learning' | 'assignments' | 'research' | 'profile'
  >('overview');
  ...
  const [mySubmissions, setMySubmissions] = useState<StudentSubmission[]>([
    {
      id: 'stud_sub_01',
      assignmentTitle:
        'Constitutional Law I: Separation of Powers and Supremacy of Constitution Analysis',
      courseOrUnit: 'Constitutional Law I',
      submissionNotes:
        'Legal essay evaluating Articles 1, 2, 79, 98, and 126 of the 1995 Constitution.',
      ...
      grade: '88% (A)',
      lecturerFeedback:
        'Excellent analysis of Article 137(1) vs 137(3) jurisdiction. Well argued.',
  ...
  const [selectedSubForPreview, setSelectedSubForPreview] =
  ...
  const fetchMySubmissions = async () => {
    ...
    try {
      ...
      const res = await getStudentSubmissions(email, 'Student');
      if (res.data && Array.isArray(res.data.studentSubmissions)) {
        setMySubmissions(res.data.studentSubmissions);
      }
    } catch {
      // Retain offline state quietly
    } finally {
  ...
  const handleUploadAssignment = async (e: React.FormEvent) => {
    ...
    try {
      const res = await submitStudentAssignment({
        assignmentTitle: sanitizeInput(subTitle.trim()),
        courseOrUnit: subCourse,
        submissionNotes: sanitizeInput(subNotes.trim()),
      ...
      if (!res.error) {
        setSubmissionSuccessMsg(
          'Assignment submitted to faculty successfully! Awaiting lecturer review.',
        );
        ...
        setSubmissionErrorMsg(res.error || 'Failed to submit assignment.');
      }
    } catch {
  ...
  const handleWithdrawAssignment = async (id: string) => {
    ...
    try {
      const res = await deleteStudentSubmission(id);
      if (!res.error) {
  ...
  const completedQuizHistory = [
    {
      title: 'Constitutional Fundamental Rights (Art 20-45)',
      score: '95%',
      date: 'Yesterday',
      total: '20/20',
    },
    {
      title: 'Civil Procedure Pleadings & Plaint Drafting',
      score: '88%',
      date: '3 days ago',
      total: '18/20',
    },
    {
      title: 'Land Act Section 39 Marital Consent',
      score: '90%',
      date: '5 days ago',
      total: '18/20',
    },
    {
      title: 'Contracts Act Consideration & Offer',
      score: '82%',
      date: '1 week ago',
      total: '16/20',
    },
  ...
  const recentlyViewedMaterials = [
    {
      ...
      section: 'Section 10 - Valid Contracts',
      date: '2 hours ago',
    },
    {
      type: 'Case',
      title: 'Grace Ibingira & Ors v Uganda [1966] EA 306',
      section: 'Supreme Court - Habeas Corpus Ratio',
      date: '5 hours ago',
    },
    {
      type: 'Article',
      title: '1995 Constitution of Uganda - Article 137',
      section: 'Constitutional Court Jurisdiction',
      date: 'Yesterday',
    },
    {
      type: 'Regulation',
      title: 'Civil Procedure Rules (S.I. 71-1)',
      section: 'Order 6 - Pleadings Generally',
      date: '2 days ago',
    },
  ];
  ...
  const savedCases = [
    {
      name: 'Grace Ibingira & Ors v Uganda [1966] EA 306',
      topic: 'Constitutional Law',
      ratio: 'Prerogative writs and illegal detention under executive orders.',
    },
    {
      name: 'Major General David Tinyefuza v Attorney General (1997)',
      topic: 'Human Rights',
      ratio: 'Resignation from armed forces and fundamental freedoms under Article 22.',
    },
    {
      name: 'Charles Onyango Obbo & Anor v Attorney General (2004)',
      topic: 'Free Expression',
      ratio: 'Unconstitutionality of false news provisions under Penal Code.',
    },
  ];
  ...
  const quickNavCards = [
    { label: 'My Courses', icon: BookOpen, desc: '32 curriculum units & notes', tab: 'courses' },
    {
      label: 'Legal Library',
      icon: Scale,
      desc: 'Statutes, precedents & glossary',
      tab: 'research',
    },
    {
      label: '1995 Constitution',
      icon: Landmark,
      desc: 'All 19 chapters & articles',
      tab: 'constitution',
    },
    {
      label: 'AI Legal Tutor',
      icon: Sparkles,
      desc: 'IRAC reasoning & problem tutor',
      tab: 'tutor',
    },
    {
      label: 'Assignments',
      icon: FilePlus,
      desc: 'Submit work & view grades',
      action: () => setActiveDashTab('assignments'),
    },
    {
      label: 'Quizzes & Practice',
      icon: Calendar,
      desc: 'Timed multiple-choice tests',
      tab: 'quizzes',
    },
    {
      label: 'Drafting Suite',
      icon: PenTool,
      desc: 'Plaints, affidavits & covenants',
      tab: 'drafting',
    },
    {
      label: 'Past Exam Papers',
      icon: FileText,
      desc: 'Model answer structures',
      tab: 'past_papers',
    },
  ];
    ...
    <div className="relative space-y-6 pb-12 text-slate-100">
      ...
      <div className="relative z-10 bg-slate-950/40 border border-white/10 rounded-3xl p-5 sm:p-7 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 shadow-xl backdrop-blur-xl">
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-slate-400">
            Private Academic LMS &bull; Account:{' '}
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="flex items-center gap-3 bg-black/30 border border-white/10 px-4 py-2.5 rounded-2xl backdrop-blur-md">
            ...
            <div className="text-center px-1.5">
              <div className="text-[#c89d42] font-extrabold text-base">
                <span>{savedNotes.length || user.savedNotesCount}</span>
              </div>
          ...
          <button
            ...
            className="px-3.5 py-2.5 rounded-2xl bg-white/[0.04] border border-white/10 hover:border-[#c89d42]/40 text-slate-200 text-xs font-bold transition flex items-center gap-1.5 cursor-pointer backdrop-blur-sm active:scale-[0.98]"
          >
      ...
      <div className="relative z-10 flex items-center gap-2 border-b border-white/10 pb-3 overflow-x-auto">
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 whitespace-nowrap cursor-pointer backdrop-blur-sm active:scale-[0.98] ${
          ...
          {mySubmissions.length > 0 && (
            <span
              className={`px-1.5 py-0.5 rounded-full text-[9px] font-black ${
                activeDashTab === 'assignments'
                  ? 'bg-[#050811] text-[#c89d42]'
                  : 'bg-black/50 text-[#c89d42]'
              }`}
            >
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="bg-slate-950/40 border border-white/10 rounded-3xl p-5 sm:p-6 shadow-md space-y-3.5 backdrop-blur-md">
            ...
            <form onSubmit={handleAskQuickQuery} className="flex gap-2">
              ...
              <button
                type="submit"
                className="px-4 py-2.5 rounded-xl bg-[#c89d42] text-[#050811] font-bold text-xs hover:bg-[#dfb858] transition flex items-center gap-1.5 whitespace-nowrap shadow-sm cursor-pointer active:scale-[0.98]"
              >
            ...
            <div className="flex flex-wrap items-center gap-1.5 pt-0.5 text-[11px]">
              ...
              {[
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-white/[0.03] hover:bg-white/[0.08] text-slate-300 hover:text-[#c89d42] border border-white/10 transition text-[10px] cursor-pointer"
                >
                  {promptText}
                </button>
              ))}
              ...
              {courses.slice(0, 6).map((crs, idx) => (
                ...
                >
                  <div className="space-y-1.5">
                    ...
                    <h3 className="font-heading font-bold text-xs text-slate-100 line-clamp-1">
                      {crs.title}
                    </h3>
            ...
            <div className="p-5 bg-slate-950/40 border border-white/10 rounded-2xl space-y-3 backdrop-blur-md">
              ...
              <div className="space-y-2">
                {completedQuizHistory.slice(0, 3).map((quiz, idx) => (
                  <div
                    key={idx}
                    className="p-2.5 bg-black/30 rounded-xl border border-white/10 flex items-center justify-between text-xs backdrop-blur-sm"
                  >
                    <div className="min-w-0 pr-2">
                      <p className="font-semibold text-slate-200 truncate">{quiz.title}</p>
                      <p className="text-[10px] text-slate-400">
                        {quiz.date} &bull; Score {quiz.total}
                      </p>
                    </div>
        ...
        <div className="relative z-10 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-950/40 border border-white/10 rounded-3xl p-5 sm:p-6 shadow-md backdrop-blur-md">
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-heading font-bold text-xs transition flex items-center gap-1.5 shadow-sm whitespace-nowrap cursor-pointer active:scale-[0.98]"
            >
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-slate-400">
                          Assigned to:{' '}
                          <strong className="text-slate-200">{sub.lecturerName}</strong> (
                          {sub.lecturerEmail})
                        </p>
                      </div>
                      ...
                      {sub.grade && (
                        <div className="px-3.5 py-1.5 rounded-xl bg-black/40 border border-[#c89d42]/30 text-right">
                          ...
                          <span className="font-mono font-black text-sm text-[#c89d42]">
                            {sub.grade}
                          </span>
                        </div>
                      )}
                    ...
                    {sub.submissionNotes && (
                      <p className="text-xs text-slate-300 bg-black/30 p-3 rounded-xl border border-white/10 leading-relaxed">
                        <strong className="text-slate-400">Submission Note:</strong>{' '}
                        {sub.submissionNotes}
                      </p>
                    )}
                    ...
                    <div className="pt-2 border-t border-white/10 flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2 text-slate-400 text-[11px]">
                        ...
                        <span className="truncate max-w-[200px]">
                          {sub.fileName || 'Attached_Assignment.pdf'}
                        </span>
        ...
        <div className="relative z-10 space-y-6">
          <div className="bg-slate-950/40 border border-white/10 rounded-3xl p-5 sm:p-6 space-y-4 backdrop-blur-md">
            ...
            <div className="space-y-3">
              {courses.slice(0, 6).map((crs, idx) => (
                <div
                  key={crs.id ? `${crs.id}-${idx}` : `crs-prog-${idx}`}
                  className="p-3.5 bg-black/30 rounded-2xl border border-white/10 space-y-2 backdrop-blur-sm"
                >
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-bold text-slate-200">
                      {crs.code} - {crs.title}
                    </span>
                    <span className="text-[10px] text-[#c89d42] font-semibold">
                      {70 + idx * 5}% Progress
                    </span>
                  </div>
          ...
          <div className="bg-slate-950/40 border border-white/10 rounded-3xl p-5 sm:p-6 space-y-4 backdrop-blur-md">
            ...
            <div className="space-y-2.5">
              {savedCases.map((cs, idx) => (
                <div
                  key={idx}
                  className="p-3.5 bg-black/30 rounded-2xl border border-white/10 space-y-1 backdrop-blur-sm"
                >
                  <div className="flex justify-between items-start">
                    <p className="font-bold text-xs text-slate-100">{cs.name}</p>
                    <span className="text-[9px] font-mono text-[#c89d42] bg-white/[0.05] px-2 py-0.5 rounded border border-white/10">
                      {cs.topic}
                    </span>
                  </div>
        ...
        <div className="relative z-10 space-y-5">
          <div className="bg-slate-950/40 border border-white/10 rounded-3xl p-5 sm:p-6 space-y-4 backdrop-blur-md">
            ...
            <form onSubmit={handleSaveProfileSettings} className="space-y-3.5 text-xs">
              ...
              <button
                type="submit"
                className="px-4 py-2.5 rounded-xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-bold text-xs transition cursor-pointer active:scale-[0.98]"
              >
      ...
      {isSubmitModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-[#090f1e]/95 border border-white/10 rounded-3xl p-6 sm:p-7 max-w-lg w-full space-y-4 shadow-2xl relative max-h-[90vh] overflow-y-auto backdrop-blur-2xl">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <FilePlus className="w-5 h-5 text-[#c89d42]" />
                <h3 className="font-heading font-bold text-base text-slate-100">
                  Submit Coursework to Faculty
                </h3>
              </div>
              <button
            ...
            <form onSubmit={handleUploadAssignment} className="space-y-3.5 text-xs">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setSubLecturerName(
                      e.target.value.includes('kaggwa')
                        ? 'Dr. Apollo Kaggwa'
                        : 'Dr. Sarah Namubiru',
                    );
                  }}
                  className="w-full bg-[#090f1e] border border-white/10 rounded-xl px-3.5 py-2.5 text-slate-100 focus:outline-none focus:border-[#c89d42]"
                >
                  <option value="apollo.kaggwa@lawhub.ug">
                    Dr. Apollo Kaggwa (Constitutional Law & Civil Procedure)
                  </option>
                  <option value="sarah.namubiru@lawhub.ug">
                    Dr. Sarah Namubiru (Contracts & Commercial Law)
                  </option>
                </select>
              </div>
              ...
              <div className="pt-2 flex items-center justify-end gap-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-heading font-bold text-xs transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50 active:scale-[0.98]"
                >
                  {isSubmitting ? (
      ...
      {selectedSubForPreview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-[#090f1e]/95 border border-white/10 rounded-3xl p-6 sm:p-7 max-w-lg w-full space-y-4 shadow-2xl relative max-h-[85vh] overflow-y-auto backdrop-blur-2xl">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[10px] font-mono text-[#c89d42] font-bold">
                  {selectedSubForPreview.courseOrUnit}
                </span>
                <h3 className="font-heading font-bold text-sm text-slate-100">
                  {selectedSubForPreview.assignmentTitle}
                </h3>
              </div>
              <button
            ...
            <div className="flex justify-end">
              <button
                onClick={() => setSelectedSubForPreview(null)}
                className="px-4 py-2 rounded-xl bg-white/[0.05] border border-white/10 text-slate-200 text-xs font-semibold cursor-pointer active:scale-[0.98]"
              >
                Close preview
              </button>
            </div>
