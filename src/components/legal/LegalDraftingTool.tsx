// ─────────────────────────────────────────────────────────────
// LawHub Uganda — Legal Component: LegalDraftingTool
// AI Legal Document Drafting Assistant with statutory templates,
// variable interpolation, and professional disclaimers.
// ─────────────────────────────────────────────────────────────
...
import { draftLegalDocument } from '../../services/api';

export function LegalDraftingTool() {
  const [selectedTemplate, setSelectedTemplate] = useState<LegalDraftTemplate>(
    legalDraftTemplates[0],
  );
  ...
  const handleGenerateDocument = async () => {
    ...
    try {
      ...
      const res = await draftLegalDocument({
      ...
      if (res.data?.documentText) {
        setGeneratedDraft(res.data.documentText);
      } else {
    ...
    <div className="relative space-y-8 pb-12 text-slate-100">
      ...
      <div className="relative z-10 bg-slate-950/40 border border-white/10 rounded-3xl p-6 sm:p-8 space-y-4 shadow-xl backdrop-blur-xl">
        ...
        <EducationalDisclaimer
          variant="prominent"
          customText="AI-generated legal documents are provided strictly for educational and preliminary drafting reference purposes. Under the Advocates Act Cap 267, all documents intended for court submission or execution must be reviewed and endorsed by a qualified Ugandan advocate holding a valid practicing certificate."
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-slate-950/40 border border-white/10 rounded-3xl p-6 space-y-6 shadow-xl backdrop-blur-md">
            ...
            {legalDraftTemplates.map((tmpl) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[9px] uppercase px-1.5 py-0.5 rounded bg-white/[0.05] text-slate-400 border border-white/10">
                    {tmpl.category}
                  </span>
                </div>
          ...
          <button
            ...
            className="w-full py-3.5 rounded-2xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-bold text-xs transition shadow-lg flex items-center justify-center gap-2 cursor-pointer active:scale-[0.98]"
          >
            {isGenerating ? (
        ...
        <div className="lg:col-span-2 bg-slate-950/40 border border-white/10 rounded-3xl p-6 space-y-4 shadow-xl flex flex-col justify-between backdrop-blur-md">
              ...
              {generatedDraft && (
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-white/[0.05] hover:bg-white/10 border border-white/10 text-slate-200 font-bold text-xs transition flex items-center gap-1.5 cursor-pointer active:scale-[0.98]"
                  >
                    {copied ? (
                  ...
                  <a
                    ...
                    className="px-3 py-1.5 rounded-xl bg-[#c89d42] hover:bg-[#dfb858] text-[#050811] font-bold text-xs transition flex items-center gap-1.5 cursor-pointer active:scale-[0.98]"
                  >
              ...
              {generatedDraft ? (
                ...
                <div className="h-full min-h-[380px] flex flex-col items-center justify-center text-center space-y-3 text-slate-400">
                  ...
                  <p className="text-[11px] max-w-sm text-slate-400">
                    Fill in the party details on the left and click "Generate legal document" to build a statutory draft.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="pt-2 flex items-center justify-between text-[10px] text-slate-400 border-t border-white/10">
            ...
            <span className="flex items-center gap-1 text-emerald-400">
              <ShieldCheck className="w-3.5 h-3.5" /> Advocates Act Compliant Notice
            </span>
          </div>
        </div>
      </div>
